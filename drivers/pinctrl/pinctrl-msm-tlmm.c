...
//zte_pm liyf 20141124
#if defined(CONFIG_DEBUG_FS)		//zte jiangfeng add
	...
	for (i = 0; i < pinfo->num_pins; i++) {
		...
		if (!dir) {
			...
			if (int_en) {
				...
				switch (int_owner) {
				case 0x0:
					len += sprintf(list_gpio + len, "WCSS, ");
					break;
				case 0x1:
					len += sprintf(list_gpio + len, "SENSORS, ");
					break;
				case 0x2:
				...
				case 0x3:
					...
					break;
				case 0x4:
					len += sprintf(list_gpio + len, " APSS, ");
					break;
				case 0x5:
					len += sprintf(list_gpio + len, "MSS, ");
					break;
				case 0x6:
					len += sprintf(list_gpio + len, "TZ, ");
					break;
				case 0x7:
...
static int msm_tlmm_probe(struct platform_device *pdev)
{
	...
	int result;		//zte_pm liyf 20141124
	...
	gpio_status_debug_init(tlmm_pininfo);		//zte jiangfeng add
	return result;
	//zte_pm end
}
